...
import { PositionYieldDataKey, PositionYieldDataForToken, PositionYieldDataForTokenLibrary, CustomUintSizeChecks } from "../../../../src/vault/types/PositionYieldDataForToken.sol";
...
contract PositionYieldDataForTokenTest is PRBTest, StdUtils {
  ...
  mapping(PositionYieldDataKey => PositionYieldDataForToken) internal _positionYieldData;  

  function test_update_RevertWhen_BalanceIsTooBig() public {
    vm.expectRevert(
      abi.encodeWithSelector(CustomUintSizeChecks.UintOverflowed.selector, 2**150, 2**150-1)
    );
  ...
  function test_update_RevertWhen_AccumIsTooBig() public {
    vm.expectRevert(
      abi.encodeWithSelector(CustomUintSizeChecks.UintOverflowed.selector, 2**102, 2**102-1)
    );
    _positionYieldData.update({
      positionId: POSITION_ID,
      token: TOKEN,
  ...
  function test_update_RevertWhen_LossEventsIsTooBig() public {
    vm.expectRevert(
      abi.encodeWithSelector(CustomUintSizeChecks.UintOverflowed.selector, 2**4, 2**4-1)
    );
    _positionYieldData.update({
      positionId: POSITION_ID,
      token: TOKEN,
  ...
  function testFuzz_update(uint152 accumulator, uint104 totalBalance, uint8 lossEvents) public {
    ...
    (uint256 yieldAccumulator, uint256 lastRecordedTotalBalance, uint256 totalLossEvents) = _positionYieldData.read(POSITION_ID, TOKEN);
