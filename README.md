...
When a user first deposit's into Earn, a "position" will be created to track their yield over time. Positions are
...
sure that users can only access their own funds. The vault will also keep track of fees generated by Balmy

### Position

In order to use Earn, users will have to create a position. A position simply keeps track of the funds deposited and
earned by the user, in the context of a specific strategy. This means that when a position is created, it can't change
the chosen strategy later.
...
Each strategy will have its own logic and risks associated with it. They might implement leverage or maybe have some
...
update". Then, after a certain delay has passed, then the owner will be able to execute the update. As part of the
process, the old vault will be told by the registry that it should migrate all funds over to the new strategy.

It's important to note that even though there is this delayed upgrade mechanism in place, a strategy might simply
...
Like we said before, sometimes farms can implement a lock up period for withdrawals. In that case, when a user wants to
execute a withdrawal, the strategy will call a "delayed withdrawal" adapter to handle this process. It goes without
saying that each "delayed withdrawal" will be associated to a position so that only accounts with permissions can take
the funds later
...
flowchart
    ...
    admin --->|Collects Fees|vault
    admin --->|Pauses deposits|vault
    admin --->|Updates Fees|fee
